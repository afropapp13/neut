...
int neutfill_(char * /* filename */, char * /* treename */,
              char * /* Vector Branch name */, char * /* Vertex Branch name */,
              int, int, int, int);
...
int neutfill_(char *filename, char *treename, char *vecbranchname,
              char *vtxbranchname, int len_fname, int len_tname,
...
int neutfill(char *filename, char *treename, char *vecbranchname,
             char *vtxbranchname) {
  NeutRootHandlers nrh;
  static TTree *t = NULL;
...
int neutfillvect_(char *filename, char *treename, char *branchname,
...
int neutfillvect(char *filename, char *treename, char *branchname) {
  ...
  // Single pion harmonic oscillator scaling (electron-scattering like or
  // neutrino-scattering like) These are the parameters _actually_ used in
  // Rein-Sehgal code, not the neut1pi common block
  ...
  for (i = 0; i < np; i++) {
    ...
    pinfo.fPosFin.SetXYZT(posinnuc_.posnuc[i][0], posinnuc_.posnuc[i][1],
                          posinnuc_.posnuc[i][2], 0.);
  ...
  if (nfsip) {
    ...
    for (i = 0; i < nfsip; i++) {
      ...
      fsipinfo.fDir.SetXYZT(fsihist_.dirvert[i][0], fsihist_.dirvert[i][1],
                            fsihist_.dirvert[i][2], 0.);
  ...
  if (nnucfsivert) {
    ...
    for (i = 0; i < nnucfsivert; i++) {
      ...
      nucfsivinfo.fPos.SetXYZT(nucleonfsihist_.nfx[i], nucleonfsihist_.nfy[i],
                               nucleonfsihist_.nfz[i], 0.);
      nucfsivinfo.fMom.SetPxPyPzE(
          nucleonfsihist_.nfpx[i], nucleonfsihist_.nfpy[i],
          nucleonfsihist_.nfpz[i], nucleonfsihist_.nfe[i]);
...
int neutfillvtx_(char *filename, char *treename, char *branchname,
                 int len_fname, int len_tname, int len_bname) {
  NeutRootHandlers nrh;
...
int neutfillvtx(char *filename, char *treename, char *branchname) {
  ...
  for (i = 0; i < nvtx; i++) {
    tlv.SetXYZT(vcvrtx_.pvtxvc[i][0], vcvrtx_.pvtxvc[i][1],
                vcvrtx_.pvtxvc[i][2], vcvrtx_.timvvc[i]);
