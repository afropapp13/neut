...
CascadePionEngine::CascadePionEngine() { 
  FrAbs_pi = RegisterDial("FrAbs_pi", neffpr_.fefabs);
  FrInelLow_pi = RegisterDial("FrInelLow_pi", neffpr_.fefqe);
  FrCExLow_pi = RegisterDial("FrCExLow_pi", neffpr_.fefcx);
  FrInelHigh_pi = RegisterDial("FrInelHigh_pi", neffpr_.fefqeh);
  FrCExHigh_pi = RegisterDial("FrCExHigh_pi", neffpr_.fefcxh);
  FrPiProd_pi = RegisterDial("FrPiProd_pi", neffpr_.fefinel);
}
...
double CascadePionEngine::CalcWeight() {
  ...
  if (old_xsec <= 0) {
    std::cout << "[WARN]: CascadePionEngine() evpiprob old_xsec <= 0, returning "
                 "weight = 1"
              << std::endl;
    return 1;
  }
