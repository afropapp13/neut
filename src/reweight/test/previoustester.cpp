...
void FillNeutCommons(NeutVect* nvect) {
  ...
  */
    ...
    for (int j =0; j<nucleonfsihist_.nfnvert; j++)
    {
      ...
      nucleonfsihist_.nfecms2[j] = nvect->NucFsiStepInfo(j)->fECMS2;
  ...
  if ((int)nvect->NfsiVert() ==      1) 
    {  // An event with FSI must have at least two vertices                                                                                                     
