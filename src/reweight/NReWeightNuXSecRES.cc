...
double NReWeightNuXSecRES::CalcWeightMaMv() 
{
  ...
  if (old_xsec==0) {
#ifdef _N_REWEIGHT_RES_DEBUG_
    cout << "NReWeightNuXSecRES::CalcWeightMa() Warning: old_xsec==0, setting weight to 1" << '\n';
#endif
    return 1;
  }
  ...
  if(fMode==kModeNormAndMaShape) {
    ...
    if (nework_.ipne[0] == 14) inu = neutTotCrs->numu;
      ...
      std::cerr << "NReWeightNuXSecRES::CalcWeightMa() Error: Cannot MA-shape reweight this neutrino type = " 
	        << nework_.ipne[0] << '\n';
      exit (-1);
    }
    ...
    if (new_tot_xsec==0) {
#ifdef _N_REWEIGHT_RES_DEBUG_
      cout << "NReWeightNuXSecRES::CalcWeightMa() Warning: new_tot_xsec==0, setting weight to 1" << '\n';
#endif
      return 1;
    }
...
#ifdef _N_REWEIGHT_RES_DEBUG_
    cout << "NReWeightNuXSecRES::CalcWeightMa() Warning: new_weight is infinite, setting to 1" << '\n';
#endif
    new_weight = 1;
  }
